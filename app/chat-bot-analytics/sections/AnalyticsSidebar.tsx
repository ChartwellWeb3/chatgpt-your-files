...
export function AnalyticsSidebar({ links }: { links: AnalyticsLink[] }) {
  ...
  return (
    <>
      ...
      <div
        className={`absolute left-0 top-0  h-full ${open ? "z-40" : "-z-10"}`}
      >
        <div className="relative h-full">
          <aside
            className={`h-full w-56 border-r border-border bg-card px-4 py-6 shadow-lg transition-transform duration-200 ${open ? "translate-x-0" : "-translate-x-full"}`}
          >
