-- Returns a conversion profile: intent, sentiment, satisfaction, lang, and page-type
-- breakdown for visitors who submitted a chat_bot_book_a_tour form.
--
-- p_start / p_end filter visitor_forms.submitted_at (when the booking happened).
-- The latest chat_visitor_analyses row per booker is used for AI metrics.
...
with
  ...
  booker_langs as (
    select
      ...
      count(*) filter (where lang not like 'en%'
                       and  lang not like 'fr%')::int as other
    from (
  ...
  -- page type split from analyses
  booker_page_types as (
    select
      count(*) filter (where page_type = 'corporate')::int        as corporate,
      count(*) filter (where page_type = 'residence')::int        as residence,
      count(*) filter (where page_type = 'find_a_residence')::int as find_a_residence,
      count(*) filter (where page_type not in ('corporate','residence','find_a_residence')
                       or page_type is null)::int                   as unknown
    from booker_analyses
  )
...
select jsonb_build_object(
  ...
  'lang_split', jsonb_build_object(
    ...
    'other', (select other from booker_langs)
  ),
  'page_type_split', jsonb_build_object(
    'corporate',       (select corporate       from booker_page_types),
    'residence',       (select residence       from booker_page_types),
    'find_a_residence',(select find_a_residence from booker_page_types),
    'unknown',         (select unknown         from booker_page_types)
  )
);
$$;
