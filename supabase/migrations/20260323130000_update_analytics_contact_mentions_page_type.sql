-- Recreate analytics_contact_mentions with p_page_type filter (corporate / residence).
-- Corporate sessions have residence_custom_id in ('corporateen', 'corporatefr').
-- Residence sessions are everything else.
...
revoke all on function public.analytics_contact_mentions(date, date, int, text) from public;
grant execute on function public.analytics_contact_mentions(date, date, int, text) to authenticated;
