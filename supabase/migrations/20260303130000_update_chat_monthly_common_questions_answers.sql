drop function if exists public.chat_monthly_common_questions(date, date, int);
drop function if exists public.chat_monthly_common_questions(date, date, text, int);

create function public.chat_monthly_common_questions(
  p_start date,
  p_end date,
  p_lang text,
...
as $$
  ...
  normalized as (
    select
      ...
      position('?' in coalesce(user_content, '')) > 0 as has_question_mark
    from filtered
  ),
  ranked as (
    select
