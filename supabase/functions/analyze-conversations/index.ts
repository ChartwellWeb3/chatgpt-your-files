...
async function runAnalysis(opts: {
  ...
  const missedOrWeak = rawMissed
    .map((entry: unknown) => {
      ...
      return {
        visitor_question: asString(record.visitor_question).trim(),
        assistant_response: asString(record.assistant_response).trim(),
        issue_type: normalizeIssueType(record.issue_type),
        why_insufficient: asString(record.why_insufficient).trim(),
      };
    })
    .filter(
      (entry): entry is {
        ...
        if (!sessErr) {
          ...
          const residenceCustomId =
            typeof session?.residence_custom_id === "string"
              ...
              : "";
          const pageUrl =
            typeof session?.page_url === "string" ? session.page_url : "";
